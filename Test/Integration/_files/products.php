...
$product->setTypeId(\Magento\Catalog\Model\Product\Type::TYPE_SIMPLE)
    ->setId(610)
    ->setAttributeSetId(4)
    ->setName('Active offer no qty with backorders')
    ->setSku('active_offer_no_qty_with_backorders')
    ->setUrlKey('active_offer_no_qty_with_backorders')
    ->setPrice(10)
    ->setVisibility(\Magento\Catalog\Model\Product\Visibility::VISIBILITY_BOTH)
    ->setStatus(\Magento\Catalog\Model\Product\Attribute\Source\Status::STATUS_ENABLED)
    ->setWebsiteIds([1])
    ->setStockData(['use_config_manage_stock' => 1, 'qty' => 0, 'is_qty_decimal' => 0, 'is_in_stock' => 1, 'backorders' => 1])
    ->setCanSaveCustomOptions(true)
    ->setSpecialPrice(7)
    ->setDailyDealEnabled(1)
    ->setDailyDealLimit(50)
